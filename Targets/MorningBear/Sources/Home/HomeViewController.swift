...
extension HomeViewController {
    ...
    private func properHeaderCell(for indexPath: IndexPath) -> HomeSectionHeaderCell {
        ...
        switch HomeSection.getSection(index: indexPath.section) {
        case .recentMornings:
            headerCell.prepare(descText: nil, titleText: "나의 최근 미라클모닝")
        case .badges:
            headerCell.prepare(descText: "모두 보기>", titleText: "내가 모은 배지") {
                print("내가 모은 배지")
            }
        case .articles:
            headerCell.prepare(descText: "모두 보기>", titleText: "읽을거리") {
                print("읽을거리")
            }
        default:
            break
