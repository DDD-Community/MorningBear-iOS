...
public struct BadgeStateDataProvider: DataProviding {
    public func fetch() -> Single<[Badge]> {
        return fetch(BadgeQuery())
    }
    
    func fetch(_ model: BadgeQuery) -> Single<[Badge]> {
        Observable.of(
            [
    ...
    public struct BadgeQuery: Queryable {}
}
