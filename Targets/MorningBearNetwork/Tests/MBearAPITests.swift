...
typealias EndpointClosure<T: TargetType> = (T) -> Endpoint
typealias StubClosure<T: TargetType> = MoyaProvider<T>.StubClosure

final class MBearAPITests: XCTestCase {
    ...
    func test__request_get_300() throws {
        let expectation = XCTestExpectation(description: "Network")
        let errorStubProvider = stubProvider(endpointClosure: mBearEndpointClosure(300))
